...
const Hint = ({ setDisabled, disabled }: HelpProps) => {
	...
	return (
		<motion.div initial={{ opacity: 0 }} transition={{duration: .5, delay: 2}} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className={clsx(disabled?"flex":"hidden", "fixed inset-0 bg-black/70 z-50  items-center justify-center px-4")}>
			<motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-zinc-800 rounded-lg p-6 max-w-[24rem] w-full relative">
				<button
				...
				<div className="space-y-6">
					<div>
						...
						<ul className="text-white space-y-2 list-disc list-inside">
							...
							<li>Hit <span className="bg-[var(--purple)] text-black p-1 rounded">ENTER</span> to submit your guess</li>
					...
					<div>
						...
						<div className="space-y-3">
							<div className="flex items-center gap-3">
								<div className="w-8 h-8 bg-green-500 rounded flex items-center justify-center text-zinc-800 font-bold">
									A
								</div>
								<span className="text-white">Letter is correct and in the right spot</span>
							</div>
							<div className="flex items-center gap-3">
								<div className="w-8 h-8 bg-yellow-500 rounded flex items-center justify-center text-zinc-800 font-bold">
									B
								</div>
								<span className="text-white">Letter in the word but in wrong spot</span>
							</div>
							<div className="flex items-center gap-3">
								<div className="w-8 h-8 bg-red-500 rounded flex items-center justify-center text-zinc-800 font-bold">
									C
								</div>
					...
					<div className="text-white mt-6">
						...
						<ul className="space-y-2 list-disc list-inside">
							...
							<li>Press <span className="bg-red-500 rounded p-1 text-black">DEL</span> to remove letters</li>
						</ul>
					</div>
				</div>
				...
				<button
					onClick={() => setDisabled(false)}
					className="mt-8 w-full py-3 bg-[var(--purple)] text-zinc-800 rounded-md font-semibold"
				>
					Got it!
				</button>
			</motion.div>
